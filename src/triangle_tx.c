...
static void     set_coordinates(sfVector2f *arr, sfVector3f *pos)
{
    arr[0].x = pos[0].x;
    arr[0].y = pos[0].y;
    arr[1].x = pos[1].x;
...
static vector4f_t       calc_normal(sfVector3f *pos)
{
...
static void     triangle_filler(sfVector3f *pos, sfVector2f *arr, sfColor color,
my_game_t *game)
{
...
arr[4].x += pos[3].y, arr[3].x += pos[3].x)
            horz_line_tx(game, (sfVector3f){arr[4].x, arr[3].x, arr[4].y}, nor,
color);
        arr[3] = arr[1];
